...
def md5_hash(text):
    md5_hash_output = MD5.new()
    md5_hash_output.update(text)
    return md5_hash_output.digest()

def sha1_hash(text):
    sha1_hash_output = SHA1.new()
    sha1_hash_output.update(text)
    return sha1_hash_output.digest()

def sha256_hash(text):
    sha256_hash_output = SHA256.new()
    sha256_hash_output.update(text)
